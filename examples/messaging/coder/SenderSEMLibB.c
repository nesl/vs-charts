...
#define SEM SEMSender


/* Guard expression handler */
#define VSGuard SenderVSGuard


#ifdef VS_RUNTIME_INFO
SenderVSRunTimeInfo volatile const VS_RUNTIME_INFO_EXTKW SendervsRunTimeInfo = 
{
...
#if (SEM_SIGNAL)
void SenderSEM_InitSignalQueue (void)
{
  SEM.SPut = 0;
  SEM.SGet = 0;
  SEM.SUsed = 0;
...
void SenderSEM_SignalQueueInfo (SEM_SIGNAL_QUEUE_TYPE *NofSignals)
{
...
#if (SEM_INIT_ALL)
void SenderSEM_InitAll (void)
{
  SenderSEM_Init();
#if (SEM_INIT_EXTERNAL_VARIABLES)
  SenderSEM_InitExternalVariables();
#endif
#if (SEM_INIT_INTERNAL_VARIABLES)
  SenderSEM_InitInternalVariables();
#endif
#if (SEM_SIGNAL)
  SenderSEM_InitSignalQueue();
#endif
}
#endif
...
{
#ifdef VS_RUNTIME_INFO
  *SendervsRunTimeInfo.pSignatureVersion;
#endif
...
#endif
unsigned char SenderSEM_GetOutputAll (SEM_ACTION_EXPRESSION_TYPE *ActionVector,
  SEM_ACTION_EXPRESSION_TYPE MaxSize)
{
  SEM_ACTION_EXPRESSION_TYPE i;
  unsigned char CC;

  for (i = 0; ((CC = SenderSEM_GetOutput(&ActionVector[i])) == SES_FOUND) && i < MaxSize - 1; i++);
...
#if (SEM_NEXT_STATE_CHG == 1)
unsigned char SenderSEM_NextStateChg (void)
#else
...
#endif
{
  ...
  {
    while ((CC = SenderSEM_GetOutput (&nAction)) == SES_FOUND) {
      ;
    }
    if (CC != SES_OKAY) {
      return(CC);
...
#if (SEM_NAME == 1)
unsigned char SenderSEM_Name (unsigned char IdentType, 
...
#if (SEM_NAME_ABS == 1)
unsigned char SenderSEM_NameAbs (unsigned char IdentType, 
...
#if (SEM_EXPL == 1)
unsigned char SenderSEM_Expl (unsigned char IdentType, SEM_EXPLANATION_TYPE IdentNo,
...
#if (SEM_EXPL_ABS == 1)
unsigned char SenderSEM_ExplAbs (unsigned char IdentType,
  SEM_EXPLANATION_TYPE IdentNo, char const **Text)
{
  switch (IdentType)
  {
...
#if (SEM_STATE == 1)
unsigned char SenderSEM_State (SEM_STATE_MACHINE_TYPE StateMachineNo,
...
#if (SEM_STATE_ALL == 1)
unsigned char SenderSEM_StateAll (SEM_STATE_TYPE *StateVector,
...
#if (SEM_MACHINE == 1)
unsigned char SenderSEM_Machine (SEM_STATE_TYPE StateNo,
...
unsigned char SenderSEM_ForceState (SEM_STATE_TYPE StateNo)
{
  if (VS_NOF_STATES <= StateNo) {
    return (SES_RANGE_ERR);
  }

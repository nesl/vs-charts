...
#define VS Multiplier


/* SEM data struct name */
#define SEM SEMMultiplier


#ifdef VS_RUNTIME_INFO
MultiplierVSRunTimeInfo volatile const VS_RUNTIME_INFO_EXTKW MultipliervsRunTimeInfo = 
{
...
#if (SEM_SIGNAL)
void MultiplierSEM_InitSignalQueue (void)
{
  SEM.SPut = 0;
  SEM.SGet = 0;
  SEM.SUsed = 0;
...
void MultiplierSEM_SignalQueueInfo (SEM_SIGNAL_QUEUE_TYPE *NofSignals)
{
...
#if (SEM_INIT_ALL)
void MultiplierSEM_InitAll (void)
{
  MultiplierSEM_Init();
#if (SEM_INIT_EXTERNAL_VARIABLES)
  MultiplierSEM_InitExternalVariables();
#endif
#if (SEM_INIT_INTERNAL_VARIABLES)
  MultiplierSEM_InitInternalVariables();
#endif
#if (SEM_SIGNAL)
  MultiplierSEM_InitSignalQueue();
#endif
}
#endif


void MultiplierSEM_Init (void)
{
#ifdef VS_RUNTIME_INFO
  *MultipliervsRunTimeInfo.pSignatureVersion;
#endif
...
unsigned char MultiplierSEM_GetOutput (SEM_ACTION_EXPRESSION_TYPE *ActionNo)
{
  for(;;)
  {
    switch (SEM.State)
...
#endif
unsigned char MultiplierSEM_GetOutputAll (SEM_ACTION_EXPRESSION_TYPE *ActionVector,
  SEM_ACTION_EXPRESSION_TYPE MaxSize)
{
  SEM_ACTION_EXPRESSION_TYPE i;
  unsigned char CC;

  for (i = 0; ((CC = MultiplierSEM_GetOutput(&ActionVector[i])) == SES_FOUND) && i < MaxSize - 1; i++);
...
#if (SEM_NEXT_STATE_CHG == 1)
unsigned char MultiplierSEM_NextStateChg (void)
#else
unsigned char MultiplierSEM_NextState (void)
#endif
{
  unsigned char CC;
  ...
  {
    while ((CC = MultiplierSEM_GetOutput (&nAction)) == SES_FOUND) {
      ;
    }
    if (CC != SES_OKAY) {
      return(CC);
...
#if (SEM_NAME == 1)
unsigned char MultiplierSEM_Name (unsigned char IdentType, 
...
#if (SEM_NAME_ABS == 1)
unsigned char MultiplierSEM_NameAbs (unsigned char IdentType, 
...
#if (SEM_EXPL == 1)
unsigned char MultiplierSEM_Expl (unsigned char IdentType, SEM_EXPLANATION_TYPE IdentNo,
...
#if (SEM_EXPL_ABS == 1)
unsigned char MultiplierSEM_ExplAbs (unsigned char IdentType,
  SEM_EXPLANATION_TYPE IdentNo, char const **Text)
{
  switch (IdentType)
  {
...
#if (SEM_STATE == 1)
unsigned char MultiplierSEM_State (SEM_STATE_MACHINE_TYPE StateMachineNo,
...
#if (SEM_STATE_ALL == 1)
unsigned char MultiplierSEM_StateAll (SEM_STATE_TYPE *StateVector,
...
#if (SEM_MACHINE == 1)
unsigned char MultiplierSEM_Machine (SEM_STATE_TYPE StateNo,
...
unsigned char MultiplierSEM_ForceState (SEM_STATE_TYPE StateNo)
{
  if (VS_NOF_STATES <= StateNo) {
    return (SES_RANGE_ERR);
  }
